...
export type RapidDatePickerRockConfig = RockConfig<RapidDatePickerProps, typeof RAPID_DATE_PICKER_ROCK_TYPE>;
```
...
// 注意：这里可能需要根据实际情况调整继承方式，或者重新定义 Props
export type RapidCurrencyRendererRockConfig = RockConfig<RapidCurrencyRendererProps, typeof RAPID_CURRENCY_RENDERER_ROCK_TYPE> &
  Omit<RapidNumberRendererRockConfig, "value" | "$type">;
```
...
```typescript
import { RockComponentProps } from "@ruiapp/move-style";
...
- 组件 Props 使用 `RockComponentProps<{RockName}RockConfig>`
...
export function RapidDictionaryEntryRendererComponent(props: RockComponentProps<RapidDictionaryEntryRendererRockConfig>) {
...
export function RapidDatePickerComponent(props: RockComponentProps<RapidDatePickerRockConfig>) {
  const context = useRockInstanceContext();
  const { framework, page, scope } = context;
...
export function RapidCurrencyRenderer(props: RockComponentProps<RapidCurrencyRendererRockConfig>) {
...
4. **Slots 处理**:
   ...
   export function RapidArrayRenderer(props: RockComponentProps<RapidArrayRendererRockConfig>) {
...
6. **在 React 组件中触发 RockEvent**:
   ...
   export function MyRockComponent(props: RockComponentProps<MyRockComponentProps>) {
     const context = useRockInstanceContext();
     const { framework, page, scope } = context;
   ...
   **完整示例 - SonicToolbarNewEntityButton**:
   ...
   ```typescript
   import type { Rock, RockInstanceProps, RockComponentProps } from "@ruiapp/move-style";
   import { fireEvent } from "@ruiapp/move-style";
   ...
   export function SonicToolbarNewEntityButtonComponent(props: RockComponentProps<SonicToolbarNewEntityButtonProps>) {
     const context = useRockInstanceContext();
     const { framework, page, scope } = context;
...
- 新组件应直接接收 `RockComponentProps<{RockName}RockConfig>` 类型，不包含 `context` 参数
