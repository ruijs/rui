import type { SdRpdPage as TSdRpdPage } from '~/types/sd-rapid-page-types';
import AppNavItemDetailsPage from '../models/sd-rpd-pages/AppNavItemDetailsPage';
import AppNavItemEditPage from '../models/sd-rpd-pages/AppNavItemEditPage';
import AppNavItemListPage from '../models/sd-rpd-pages/AppNavItemListPage';
import AppNavItemNewPage from '../models/sd-rpd-pages/AppNavItemNewPage';
import BaseEmployeeListPage from '../models/sd-rpd-pages/BaseEmployeeListPage';
import BaseEquipmentListPage from '../models/sd-rpd-pages/BaseEquipmentListPage';
import BaseFormFieldListPage from '../models/sd-rpd-pages/BaseFormFieldListPage';
import BaseFormFieldNewPage from '../models/sd-rpd-pages/BaseFormFieldNewPage';
import BaseMaterialDetailsPage from '../models/sd-rpd-pages/BaseMaterialDetailsPage';
import BaseMaterialListPage from '../models/sd-rpd-pages/BaseMaterialListPage';
import BaseProdFlowListPage from '../models/sd-rpd-pages/BaseProdFlowListPage';
import BaseProdProcessListPage from '../models/sd-rpd-pages/BaseProdProcessListPage';
import BaseUnitListPage from '../models/sd-rpd-pages/BaseUnitListPage';
import DataDictionaryDetailsPage from '../models/sd-rpd-pages/DataDictionaryDetailsPage';
import DataDictionaryEditPage from '../models/sd-rpd-pages/DataDictionaryEditPage';
import DataDictionaryListPage from '../models/sd-rpd-pages/DataDictionaryListPage';
import DataDictionaryNewPage from '../models/sd-rpd-pages/DataDictionaryNewPage';
import MetaModelDetailsPage from '../models/sd-rpd-pages/MetaModelDetailsPage';
import MetaModelListPage from '../models/sd-rpd-pages/MetaModelListPage';
import MetaRouteListPage from '../models/sd-rpd-pages/MetaRouteListPage';
import MomProdOrderDetailsPage from '../models/sd-rpd-pages/MomProdOrderDetailsPage';
import MomProdOrderListPage from '../models/sd-rpd-pages/MomProdOrderListPage';
import MomProdPlanDetailsPage from '../models/sd-rpd-pages/MomProdPlanDetailsPage';
import MomProdPlanListPage from '../models/sd-rpd-pages/MomProdPlanListPage';
import MomProdTaskDetailsPage from '../models/sd-rpd-pages/MomProdTaskDetailsPage';
import MomProdTaskListPage from '../models/sd-rpd-pages/MomProdTaskListPage';
import OcDepartmentListPage from '../models/sd-rpd-pages/OcDepartmentListPage';
import OcRoleListPage from '../models/sd-rpd-pages/OcRoleListPage';
import OcUserListPage from '../models/sd-rpd-pages/OcUserListPage';
import SysWebhookListPage from '../models/sd-rpd-pages/SysWebhookListPage';

export default [
  AppNavItemDetailsPage,
  AppNavItemEditPage,
  AppNavItemListPage,
  AppNavItemNewPage,
  BaseEmployeeListPage,
  BaseEquipmentListPage,
  BaseFormFieldListPage,
  BaseFormFieldNewPage,
  BaseMaterialDetailsPage,
  BaseMaterialListPage,
  BaseProdFlowListPage,
  BaseProdProcessListPage,
  BaseUnitListPage,
  DataDictionaryDetailsPage,
  DataDictionaryEditPage,
  DataDictionaryListPage,
  DataDictionaryNewPage,
  MetaModelDetailsPage,
  MetaModelListPage,
  MetaRouteListPage,
  MomProdOrderDetailsPage,
  MomProdOrderListPage,
  MomProdPlanDetailsPage,
  MomProdPlanListPage,
  MomProdTaskDetailsPage,
  MomProdTaskListPage,
  OcDepartmentListPage,
  OcRoleListPage,
  OcUserListPage,
  SysWebhookListPage,
] as TSdRpdPage[];
